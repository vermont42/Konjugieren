...
**Testing an unpublished change to the skill itself.** `ios-build-verify` is developed locally at `~/Desktop/workspace/ios-build-verify` but consumed from GitHub, so both the marketplace clone and the versioned cache hold the *published* code. To exercise local edits without cutting a release, point the variable at the dev repo — `export IBV_SCRIPTS=~/Desktop/workspace/ios-build-verify/skills/ios-build-verify/scripts` — and unset it to return to the published copy. `scripts/take_screenshots.sh` honors a pre-set `IBV_SCRIPTS` too, so one export covers both it and the commands above.
