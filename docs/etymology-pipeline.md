...
**COMPLETE — every verb in the corpus now ships an etymology in both `"en"` and `"de"`, as of 2026-07-23.** The gap this heading spent three days lying about is closed: roadmap step 10 landed all of it. `check_docs.py`'s "etymology completeness claim" check now reads this line, sees the `**COMPLETE` claim, and *actively verifies* it against `Verbs.xml` on every run — so if a future verb ever ships without an etymology, this claim fails loudly rather than going stale.

**But not by this document's workflow, which is now historical.** This file describes a *translation-only* pipeline: English etymology in, German out, 5×20 subagents. It never supplied the missing etymologies themselves. Those were mined and composed by the [`prompts/uses_etymologies.md`](../prompts/uses_etymologies.md) pipeline (Phase 4 mining → Phase 5 integrate, commit `34e57ab`), which reuses roots and prefixes rather than translating prose. The translation workflow below is kept for the record, and because it remains the right tool if a future English-only etymology ever needs a German side; it is not the thing that filled the corpus.

The cautionary tale is worth keeping compressed rather than editing away. This heading read "COMPLETE — every verb in `Verbs.xml` is translated" from 2026-07-19 to 2026-07-20 — true when written, false within hours because tranche 1 landed the same day and took the corpus 990 → 3,572. Two lines below the false headline, the same file warned against trusting a count in prose and supplied the exact coverage command that disproved it. The recipe was right and nobody ran it; that is the whole argument for `scripts/check_docs.py`, and the conditional check it grew is why this heading can now be trusted.
