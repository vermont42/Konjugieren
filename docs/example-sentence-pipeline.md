...
Suggested batching: ~25–33 subagents for 990 verbs. Run 3 in parallel per round.

### Phase 4: Medieval Pass (Top 30 Only)

**This phase never landed.** No verb in the shipping `ExampleSentences.json` has a `medieval`
key — checked 2026-07-19 — so the schema rule above describes an output that does not exist.
The list below is also frozen against the pre-2026-07-19 frequency ranks, which were hand-
assigned; ranks are now derived from DWDS hit counts and the top 30 has changed membership
(*erhalten* and *bekommen* out, *setzen* and *spielen* in). Re-derive it from
`docs/frequencies.txt` rather than reusing this list if the phase is ever run.
...
2. Validate: all 990 verbs present under both `"de"` and `"en"`
...
| `docs/frequencies.txt` | 990 verbs in frequency order; generated by `verbdata/generate_frequencies_txt.py` |
