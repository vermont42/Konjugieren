...
**Next verb number: 991** ← matches the number in column 1 of `docs/frequencies.txt` (990 verbs total). **EXTRACTION COMPLETE** — all 990 verbs have been processed.
...
7. **Update this file**: Change the "Next verb number" at the top of this file to current + (number of verbs processed). Also update the verb list under "## Task" to show the next batch of verbs from `docs/frequencies.txt`. If the new number > 990, note that extraction is complete.
