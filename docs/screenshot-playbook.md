...
- **Simulator.app running — the driver now handles this itself.** `ensure_simulator_app`
  ...
  renders black (workaround #18). Both run in a preflight pass *before* `build_app.sh`, so a
...
18. **A booted simulator can render pure black while every other signal says it is fine**
