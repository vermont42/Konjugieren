...
**The corrections are applied and shipped.** Josh reviewed the deliverable on 2026-07-29 and
directed that it be applied, so `docs/verb_history.txt` and `docs/verb_history_de.txt` carry all 27
findings, all 9 span corrections, 11 of the 16 internal-consistency items, and three repairs to the
German alone. Both were then synced into `Konjugieren/Assets/Localizable.xcstrings`, along with
Phase 0's ten patches, which had never shipped either, and the rendered essay was checked in the
running app in both languages. **This run is closed.** The follow-on work is in
[`ship-verb-history.md`](ship-verb-history.md), which is also closed, and the one open item is a new
section on the Luther Bible, briefed in [`luther-bible-section.md`](luther-bible-section.md).
