...
# sibling app Conjugar on 2026-08-01, ported here the same day; workaround #17):
#
...
#     German umlauts or ss — `axe type` fails outright on them). axe injects that combo as
...
# quiz_mid shots would alternate. (workarounds #6, #10, #16 and #17)
set_keyboard_state() {
  local want="$1" window_match attempt front
  if keyboard_state_is "$want"; then
    return 0
  fi
  # Raise the target sim's window before touching the menu. With both sims booted, the menu
  # acts on whichever window is frontmost — must be explicit. The match is by device
  # *family* substring, so it is unambiguous only while exactly one simulator per family is
  # booted (what a normal sweep produces); a stray second iPhone/iPad sim can make AXRaise
  # pick the wrong window, which the state check below is what surfaces.
  case "$DEVICE" in
    "iPhone 17 Pro Max")     window_match="iPhone" ;;
    "iPad Pro 13-inch (M4)") window_match="iPad" ;;
    *) window_match="" ;;
  esac
  # `delay 0.5`, not 0.2: with a freshly-activated Simulator the window list is briefly
  # unenumerable and AXRaise fails with -1719 "Invalid index", which reads exactly like a
  # missing-permission failure and sends you chasing the wrong thing.
  #
  ...
  # exactly when it loses. Returning non-zero here used to abort the entire sweep through
  # `set -e`, throwing away every remaining cell over one transient. A genuine permission
  # failure fails all three attempts and still gets a warning, and the state check reports
  ...
  # redundant with the AXRaise above. Retrying does not help the case it catches: when the
  # raise silently fails to take focus, a keystroke still succeeds — it just lands in
  # whichever app *is* frontmost. Observed on 2026-07-26 in the Conjugar repo, where a stray
  # Cmd+K launched the Fitness app on the host Mac while the sweep reported nothing wrong.
  # Checking frontmost first turns a silent misfire into a log line that names the app that
  # caught it. It earns its keep: during Conjugar's 2026-08-01 sweep it correctly refused to
  # click while Safari and then VS Code held focus, costing one retried cell instead of one
  # shipped defect.
  #
  ...
  # a relaunch costs nothing. (workaround #16)
...
quiz_field_value() {
  axe describe-ui --udid "$UDID" 2>/dev/null \
    | jq -r --arg id quiz_answer_field \
        '[.. | objects | select(.AXUniqueId? == $id) | .AXValue] | map(select(. != null and . != "")) | .[0] // ""' \
        2>/dev/null
}
...
# each submit, so the driver's old `tap_id quiz_answer_field` was tapping a field that
...
paste_into_quiz_field() {
  ...
  for attempt in 1 2 3; do
    case "$attempt" in
      2) tap_id_first quiz_answer_field || true ;;
      3) tap_id_first quiz_answer_field || true
...
nav_quiz_mid() {
  ...
  # which is why nothing taps it here — see paste_into_quiz_field. (workaround #17)
...
nav_quiz_results() {
  ...
  # it attached throughout. A preceding quiz_mid cell leaves it detached. (workaround #17)
  set_keyboard_state hidden
